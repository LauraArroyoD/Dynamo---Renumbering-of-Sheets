...
This automation tool for Autodesk Revit allows the re-numbering of a specific set of sheets previously selected by the user from the Project Browser. 
...
The routine begins by capturing the selection from the Project Browser. These elements are stored in a list and sorted via a custom function. The sorting logic is strictly dictated by the current **Sheet Number**, ensuring a predictable sequence before any modification occurs. 
